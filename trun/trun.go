...
// multiplication can overflow u32 so we must cast everything
// wikipedia.org/wiki/Data-rate_units
func (s *Sample) Bandwidth(media *mdhd.Box) float64 {
   return float64(media.Timescale) * float64(s.Size) * 8 / float64(s.Duration)
}
