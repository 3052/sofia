               ...
               if minf, ok := mdia.Minf(); ok {
                  if stbl, ok := minf.Stbl(); ok {
                     if stsd, ok := stbl.Stsd(); ok {
                        // Correctly check for protection by iterating through sample entries
                        // and looking for a 'sinf' box.
                        for i := range stsd.Children {
                           child := &stsd.Children[i]
                           if _, _, protected := child.Sinf(); protected {
                              isEncrypted = true
                              break
                           }
                        }
