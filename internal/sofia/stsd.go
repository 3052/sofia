...
type StsdChildBox struct {
	Encv *EncvBox
	Enca *EncaBox // Added enca
	Raw  *RawBox
}
func (c *StsdChildBox) Size() uint64 {
	if c.Encv != nil { return c.Encv.Size() }
	if c.Enca != nil { return c.Enca.Size() }
	if c.Raw != nil { return c.Raw.Size() }
	return 0
}
func (c *StsdChildBox) Format(dst []byte, offset int) int {
	if c.Encv != nil { return c.Encv.Format(dst, offset) }
	if c.Enca != nil { return c.Enca.Format(dst, offset) }
...
func ParseStsdBox(data []byte) (*StsdBox, error) {
	...
	for offset < len(data) {
		...
		child := &StsdChildBox{}
		switch header.Type {
		case "encv": child.Encv, err = ParseEncvBox(content)
		case "enca": child.Enca, err = ParseEncaBox(content)
