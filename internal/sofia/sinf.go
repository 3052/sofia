...
func (c *SinfChildBox) Size() uint64 {
	switch {
	case c.Frma != nil: return c.Frma.Size()
	case c.Schi != nil: return c.Schi.Size()
...
func (c *SinfChildBox) Format(dst []byte, offset int) int {
	switch {
	case c.Frma != nil: return c.Frma.Format(dst, offset)
	case c.Schi != nil: return c.Schi.Format(dst, offset)
...
func ParseSinfBox(data []byte) (*SinfBox, error) {
	...
	for offset < len(data) {
		...
		child := &SinfChildBox{}
		switch header.Type {
		case "frma": child.Frma, err = ParseFrmaBox(content)
		case "schi": child.Schi, err = ParseSchiBox(content)
