...
func (c *MoovChildBox) Size() uint64 {
	switch {
	case c.Trak != nil: return c.Trak.Size()
	case c.Pssh != nil: return c.Pssh.Size()
...
func (c *MoovChildBox) Format(dst []byte, offset int) int {
	switch {
	case c.Trak != nil: return c.Trak.Format(dst, offset)
	case c.Pssh != nil: return c.Pssh.Format(dst, offset)
...
func ParseMoovBox(data []byte) (*MoovBox, error) {
	...
	for offset < len(data) {
		...
		child := &MoovChildBox{}
		switch header.Type {
		case "trak": child.Trak, err = ParseTrakBox(content)
		case "pssh": child.Pssh, err = ParsePsshBox(content)
